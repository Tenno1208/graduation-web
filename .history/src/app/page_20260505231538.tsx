  ...
  useEffect(() => {
    if (hasStarted) {
      const timer = setInterval(() => {
        setIndex((prev) => (prev + 1) % 3); // Sesuaikan jumlah foto di folder public/images
      }, 5000);
    ...
    <main className="bg-black text-white selection:bg-yellow-500 selection:text-black scroll-smooth">
      ...
      {hasStarted && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
          ...
          {/* --- SECTION 1: HERO (FULLSCREEN LAPTOP, NO-CUT MOBILE) --- */}
          <section className="relative h-[100svh] w-full flex items-center justify-center overflow-hidden bg-black">
            <AnimatePresence mode="wait">
              ...
              >
                {/* Background Blur untuk mengisi layar (Standard Cinematic) */}
                <Image 
                ...
                />
                {/* Foto Utama - Full Screen di Laptop, Object Contain di Mobile agar tidak kepotong */}
                <div className="relative w-full h-full p-4 md:p-0">
                  <Image 
                    src={`/images/foto${index + 1}.jpg`} 
                    alt="Memory" 
                    fill 
                    className="object-contain md:object-cover" // Desktop: Cover (Full), Mobile: Contain (Full Foto)
                    priority
                  />
                </div>
              </motion.div>
            ...
            <div className="relative z-10 text-center px-4">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                ...
                <h1 className="text-6xl md:text-[10rem] font-serif italic leading-none opacity-90">Memories</h1>
