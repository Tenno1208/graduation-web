...
export default function GraduationPage() {
  ...
  // Auto-slide Hero Images (Sesuaikan jumlah foto di folder public/images)
  useEffect(() => {
    if (hasStarted) {
    ...
    <main className="bg-black text-white selection:bg-yellow-500 selection:text-black scroll-smooth">
      ...
      {hasStarted && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
          ...
          {/* --- SECTION 1: HERO (PERFECT FIT FOR ALL PHOTO TYPES) --- */}
          <section className="relative h-[100svh] w-full flex items-center justify-center overflow-hidden bg-black">
            <AnimatePresence mode="wait">
              ...
              >
                {/* 1. Background Blur Layer (Mengisi ruang kosong agar tidak hitam polos) */}
                <Image 
                ...
                {/* 2. Main Photo Layer (Menjamin foto utuh 100% baik Portrait/Landscape) */}
                <div className="relative w-full h-full flex items-center justify-center p-4 md:p-8">
                  <div className="relative w-full h-full">
                    <Image 
                      ...
                      alt="Memory" 
                      fill 
                      className="object-contain" // KUNCI: Foto tidak akan pernah terpotong
                      priority
                    />
                  </div>
                </div>
            ...
            <div className="relative z-10 text-center px-4">
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}>
                ...
                <h1 className="text-5xl md:text-[10rem] font-serif italic leading-none opacity-90 drop-shadow-2xl">Memories</h1>
                <p className="mt-2 md:mt-4 tracking-[0.5em] text-yellow-500 uppercase text-[10px] md:text-sm font-bold">XII TKJ 2 — Class of 2026</p>
              </motion.div>
            </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
                {students.map((student, i) => (
                  <motion.div 
                    ...
                    className="flex md:flex-col items-center md:items-start gap-4 p-4 bg-white/5 rounded-2xl md:bg-transparent border border-white/5 md:border-none shadow-lg transition-transform active:scale-95"
                  >
