...
export default function GraduationPage() {
  ...
  // Auto-slide Hero Images (Ganti 13 sesuai jumlah foto kamu)
  useEffect(() => {
    if (hasStarted) {
    ...
    <main className="bg-black text-white selection:bg-yellow-500 selection:text-black scroll-smooth">
      ...
      {hasStarted && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
          ...
          {/* --- SECTION 1: HERO (CLEAN PHOTO WITH TEXT OUTSIDE) --- */}
          <section className="relative h-[100svh] w-full flex flex-col bg-black overflow-hidden">
            
            {/* Bagian Atas: Teks Memories */}
            <div className="flex-1 flex flex-col items-center justify-end pb-4 z-10">
              <motion.div 
                key={`text-top-${index}`}
              ...
              >
                ...
                <h1 className="text-5xl md:text-8xl font-serif italic leading-none opacity-90 drop-shadow-lg text-white">
                  Memories
                </h1>
              </motion.div>
            </div>

            {/* Bagian Tengah: Foto (Utuh & Bersih) */}
                ...
                >
                  <div className="relative w-full h-full">
                    <Image 
                      ...
                      alt="Memory" 
                      fill 
                      className="object-contain" // Foto tetap utuh tanpa terpotong
                      priority
                    />
                  </div>
                </motion.div>
            ...
            <div className="flex-1 flex flex-col items-center justify-start pt-4 z-10">
              <motion.div 
