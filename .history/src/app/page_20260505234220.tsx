...
export default function GraduationPage() {
  ...
  return (
    <main className="bg-black text-white selection:bg-yellow-500 selection:text-black scroll-smooth">
      {/* Audio Element - Pastikan file memories.mp3 adalah lagu Sampai Jumpa */}
      ...
      {hasStarted && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
          ...
          {/* --- DETAILED MUSIC TOGGLE --- */}
          <div className="fixed bottom-8 right-8 z-50 flex items-center gap-3 bg-black/40 backdrop-blur-md p-2 pr-4 rounded-full border border-white/10 shadow-2xl">
            <motion.div 
              animate={{ rotate: isPlaying ? 360 : 0 }}
              transition={{ repeat: Infinity, duration: 4, ease: "linear" }}
              className="bg-yellow-500 p-2 rounded-full text-black shadow-lg"
            >
              <Music size={20} />
            </motion.div>
            
            <div className="flex flex-col">
              <span className="text-[10px] text-yellow-500 font-bold uppercase tracking-tighter leading-none">Now Playing</span>
              <span className="text-[12px] text-white font-medium truncate max-w-[100px] leading-tight">Sampai Jumpa - Endang Soekamti</span>
            </div>
            ...
            >
              {isPlaying ? <Volume2 size={20} className="text-white" /> : <VolumeX size={20} className="text-zinc-500" />}
            </button>
          </div>
          ...
          <section className="relative h-[100svh] w-full flex flex-col bg-black overflow-hidden">
            ...
            {/* Bagian Atas: Teks Memories (STATIC) & Quote (DYNAMIC) */}
            <div className="flex-1 flex flex-col items-center justify-end pb-4 z-10 px-4">
              <div className="text-center">
                ...
                {/* Teks "Memories" Statis */}
                <h1 className="text-5xl md:text-8xl font-serif italic leading-none opacity-90 drop-shadow-lg text-white mb-2">
                  Memories
                </h1>
                {/* Quote Dinamis */}
                <AnimatePresence mode="wait">
                  <motion.p 
                    ...
                    exit={{ opacity: 0, y: -5 }}
                    className="text-[10px] md:text-sm text-zinc-400 italic font-light max-w-xs md:max-w-md mx-auto"
                  >
