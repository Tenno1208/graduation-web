        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1.5 }}>
          ...
          <section className="relative h-[100svh] w-full flex flex-col bg-black overflow-hidden">
            ...
            <div className="relative w-full h-[40vh] md:h-[55vh] z-0">
              <AnimatePresence mode="wait">
                ...
                >
                  <div className="relative w-full h-full filter grayscale hover:grayscale-0 transition-all duration-1000 shadow-[0_0_50px_rgba(255,255,255,0.05)]">
            ...
            <div className="flex-1 flex flex-col items-center justify-start pt-8 z-10 px-4">
              <div className="text-center">
                ...
                <p className="text-[8px] md:text-xs text-zinc-600 uppercase tracking-widest font-bold">Generation of 2026</p>
                <ChevronDown size={20} className="mt-10 opacity-20 animate-bounce mx-auto" />
              </div>
            </div>
          </section>

          {/* --- SECTION 2: THE SQUAD (2 COLS MOBILE, 3-4 COLS DESKTOP) --- */}
            ...
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-20 gap-6">
                ...
                <div className="px-6 py-2 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-md">
                   <p className="text-zinc-400 text-[10px] md:text-xs tracking-widest uppercase font-bold">{students.length} Members</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-8">
                {students.map((student, i) => (
                  <motion.div 
                    ...
                    className="group flex flex-col bg-white/[0.02] rounded-2xl md:rounded-3xl overflow-hidden border border-white/5 hover:border-yellow-500/40 transition-all duration-500 shadow-2xl"
                  >
                    <div className="relative aspect-[3/4] w-full overflow-hidden bg-zinc-900">
                      ...
                      />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-60 md:opacity-0 group-hover:opacity-100 transition-opacity" />
                      
                      {/* Name Overlay Mobile */}
                      <div className="absolute bottom-3 left-3 right-3 md:hidden">
                        <p className="text-[10px] text-white font-bold truncate leading-none uppercase tracking-tighter mb-1">{student.name}</p>
                        <p className="text-yellow-500 text-[8px] italic font-serif line-clamp-1 opacity-80">"{student.message || 'Forever memories.'}"</p>
                      </div>
                    </div>

                    {/* Content Desktop */}
                    <div className="hidden md:flex p-6 md:p-8 flex-col flex-grow">
                      <h3 className="text-xl font-serif italic text-white mb-4 italic truncate">{student.name}</h3>
                      <div className="relative mb-6">
                        <Quote className="absolute -top-2 -left-3 text-yellow-500/20" size={24} />
                      ...
                      <div className="mt-auto pt-6 border-t border-white/5 flex items-center gap-2 text-zinc-600">
                        <MapPin size={10} className="text-yellow-500" />
                        <span className="text-[8px] tracking-[0.2em] uppercase font-black">TKJ Alumni • 2026</span>
                      </div>
                    </div>
                  </motion.div>
                ))}
          ...
          <section className="py-40 px-6 bg-black relative">
            ...
            <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="max-w-3xl mx-auto text-center">
              ...
              <div className="w-16 h-[1px] bg-yellow-500 mx-auto opacity-50" />
            </motion.div>
          </section>
          ...
          <footer className="min-h-screen flex flex-col items-center justify-center text-center p-10 bg-black relative overflow-hidden">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} whileInView={{ scale: 1, opacity: 1 }} transition={{ duration: 1.5 }}>
              ...
              <div className="space-y-4">
                 ...
                 <p className="text-yellow-600/50 text-[9px] italic font-serif">Made for XII TKJ 2 Family</p>
              </div>
            </motion.div>
          </footer>
        </motion.div>
