...
export default function GraduationPage() {
  ...
  // LOGIKA UTAMA: Loop untuk 17 Foto
  useEffect(() => {
    if (hasStarted) {
      const timer = setInterval(() => {
        // Menggunakan % 17 agar foto ke-14 sampai ke-17 muncul
        setIndex((prev) => (prev + 1) % 17);
      }, 5000);
    ...
    <main className="bg-black text-white scroll-smooth selection:bg-yellow-500 selection:text-black">
      ...
      {hasStarted && (
        ...
        >
          ...
          {/* Menampilkan Hero Section dengan proteksi quote agar tidak undefined */}
          <HeroSection 
            index={index} 
          ...
          {/* Menampilkan Quote Penutup */}
          <ClosingSection />

          {/* Menampilkan Footer Akhir */}
          <Footer />
        </motion.div>
      )}
    </main>
