...
function VideoReveal({ onClose, audioRef, setIsPlaying }: VideoRevealProps) {
  ...
  useEffect(() => {
    ...
    // Cleanup: resume musik utama saat komponen unmount (video ditutup)
    return () => {
      if (!main) return;
      main.play().then(() => {
        fadeAudio(main, "in");
  ...
  // Timers mulai setelah video siap diputar
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 flex items-center justify-center overflow-hidden bg-black py-10">
        ...
        {/* Blurred background fill (biar ga ada bar hitam jelek di samping) */}
        <div className="absolute inset-0 overflow-hidden">
          <video
            ...
            style={{ filter: "blur(24px) brightness(0.3) grayscale(80%)" }}
            aria-hidden="true"
          />
        </div>

        {/* Portrait video — rasio 9:16 */}
        <div
        ...
        <motion.div
          ...
          className="absolute top-6 inset-x-0 z-30 flex justify-center"
        >
          <div
