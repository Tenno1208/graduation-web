...
function VideoReveal({ onClose }: { onClose: () => void }) {
  ...
  // State baru untuk transisi motivasi setelah klik
  ...
  const handleContinue = () => {
    ...
    // Tunggu 5 detik di layar motivasi sebelum benar-benar kembali ke foto
    setTimeout(() => {
      onClose();
    }, 5000);
  };
  ...
  return (
    ...
    >
      {/* ─── Layar Motivasi Akhir (Muncul setelah klik) ─── */}
      <AnimatePresence>
        {isTransitioning && (
          <motion.div
            initial={{ opacity: 0 }}
