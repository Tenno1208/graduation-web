  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 overflow-hidden bg-black flex items-center justify-center">
        <video ref={videoRef} src="/video/sad.mp4" autoPlay loop playsInline onCanPlay={() => setVideoReady(true)} className="w-full h-full object-cover" style={{ filter: "grayscale(50%) brightness(0.65)" }} />
      </div>
      ...
      <div className="relative z-20 w-full text-center px-8 pb-14 pt-4 flex flex-col items-center min-h-[180px] justify-center bg-black">
        {showLines && !isTransitioning && (
          <AnimatePresence mode="wait">
            <motion.p key={lineIndex} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} className="font-serif italic text-white text-2xl">
                {lineIndex === SAD_LINES.length - 1 ? <>{SAD_LINES[lineIndex]}</> : SAD_LINES[lineIndex]}
            </motion.p>
          </AnimatePresence>
        )}
        <AnimatePresence>
          {showClose && !isTransitioning && (
            <motion.button initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} onClick={handleContinue} className="mt-10 px-8 py-4 rounded-full border border-white/10 bg-white/5 text-[9px] tracking-[0.45em] uppercase font-black text-zinc-400">
                Lanjutkan Kenangan&nbsp;→
            </motion.button>
          )}
        </AnimatePresence>
...
export default function HeroSection({ index, quote, audioRef, setIsPlaying }: HeroProps) {
  ...
  useEffect(() => {
    ...
    // Cek jika sudah melihat 20 foto
    ...
    <section ref={containerRef} className="relative min-h-[100svh] w-full bg-black overflow-hidden flex flex-col">
        ...
        <AnimatePresence mode="wait">
          <motion.div key={photoIndex} initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.98 }} transition={{ duration: 1.6, ease: [0.25, 0.1, 0.25, 1] }} className="absolute inset-0 px-4">
            <div className="relative w-full h-full rounded-2xl overflow-hidden group border border-white/5 shadow-2xl">
              <Image src={`/images/foto${photoIndex}.jpg`} alt={`Moment ${photoIndex}`} fill className="object-cover grayscale hover:grayscale-0 active:grayscale-0 transition-all duration-1000 ease-in-out" priority sizes="100vw" />
            </div>
          </motion.div>
        </AnimatePresence>
        <div className="absolute top-8 right-8 z-20 font-mono text-[9px] text-white tracking-widest bg-black/20 backdrop-blur-sm px-2 py-0.5 rounded">{String(photoIndex).padStart(2, "0")}&nbsp;/&nbsp;20</div>
      </div>
      ...
      <div className="relative z-20 px-6 pb-10 pt-6 flex flex-col items-center text-center">
        ...
        <div className="relative h-20 w-full flex flex-col items-center justify-center overflow-hidden mb-6 px-4">
          ...
          {/* Kata-kata Spesial muncul sejak AWAL selama video belum diputar */}
          {!videoFinishedPermanently && !showVideo && (
      ...
      <AnimatePresence>
        {showVideo && (
          <VideoReveal 
            ...
            onClose={() => {
              setShowVideo(false);
              setVideoFinishedPermanently(true); // Video hilang selamanya setelah diputar
            }} 
          />
        )}
      </AnimatePresence>
