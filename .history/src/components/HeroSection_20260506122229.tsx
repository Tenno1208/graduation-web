      ...
      <div className="relative z-20 px-6 pb-10 pt-6 flex flex-col items-center text-center">
        <div className="flex flex-wrap justify-center gap-1.5 mb-8 max-w-[280px]">
          {Array.from({ length: 20 }).map((_, i) => (
            <motion.span key={i} animate={{ width: i === index % 20 ? 24 : 6, backgroundColor: i === index % 20 ? "#eab308" : seenRef.current.has(i + 1) ? "rgba(234,179,8,0.3)" : "rgba(255,255,255,0.15)" }} className="block h-[2px] rounded-full" />
          ))}
        </div>
