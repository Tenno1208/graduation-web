...
function StudentCard({ student, i }: { student: any; i: number }) {
  ...
  return (
    ...
    >
      {/* Node Dot (Titik penyambung) */}
      <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-zinc-900 border border-yellow-500/40 rounded-full z-20 group-hover:bg-yellow-500 transition-colors duration-500" />
...
export default function SquadSection({ students, teacher }: SquadProps) {
  ...
  return (
    <section className="relative bg-black pt-20 pb-32 overflow-hidden">
      {/* ── Background Connected Lines ── */}
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <svg width="100%" height="100%" className="absolute inset-0">
          <pattern id="grid" width="100" height="100" patternUnits="userSpaceOnUse">
            <circle cx="50" cy="50" r="1" fill="rgba(234,179,8,0.2)" />
            <path d="M 50 0 L 50 100 M 0 50 L 100 50" stroke="rgba(234,179,8,0.05)" strokeWidth="0.5" fill="none" />
          </pattern>
        ...
        <div className="grid grid-cols-2 gap-x-4 gap-y-12">
          {students.map((student, i) => (
            <div key={student.id} className={`${i % 2 === 1 ? "mt-16" : ""}`}>
              {/* Vertical connector helper */}
              <div className="absolute left-1/2 w-px h-full bg-gradient-to-b from-yellow-500/10 to-transparent -z-10 hidden md:block" />
